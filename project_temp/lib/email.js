...
<head>
  ...
  <style>
    ...
    .cta a { background: linear-gradient(135deg, #7c3aed, #9d174d); color: white; padding: 14px 30px; border-radius: 50px; text-decoration: none; font-size: 16px; font-weight: bold; }
...
export async function sendOrderConfirmationEmail(order) {
  ...
  try {
    await transporter.sendMail({
      ...
      html: getEmailTemplate('تم تأكيد طلبك! 🎉', content, 'عرض تفاصيل الطلب', `${BASE_URL}/orders/${order.id}`),
    });
  } catch (error) {
...
export async function sendOrderDeliveredEmail(order) {
  ...
  try {
    await transporter.sendMail({
      ...
      html: getEmailTemplate('طلبك جاهز! 🎉', content, 'عرض طلباتي', `${BASE_URL}/orders`),
    });
  } catch (error) {
    console.error('Email send error:', error);
  }
...
export async function sendOrderCancelledEmail(order) {
  ...
  try {
    await transporter.sendMail({
      ...
      html: getEmailTemplate('تم إلغاء طلبك', content, 'تواصل معنا', `https://wa.me/${WHATSAPP.replace(/[^0-9]/g, '')}`),
    });
  } catch (error) {
    console.error('Email send error:', error);
  }
