...
export default function AdminOrders() {
  ...
  const getPaymentMethodLabel = (order) => {
    if (order?.paymentMethod === 'loyalty_points') return 'نقاط الولاء'
    if (order?.paymentMethod === 'paypal' || order?.paypalOrderId) return 'PayPal'
    if (order?.paymentMethod === 'free' || (order?.totalAmount || 0) === 0) return 'مجاني'
    if (order?.paymentMethod === 'manual') return 'يدوي'
    return order?.paymentMethod || 'غير محدد'
  }

  const isPaidByLoyaltyOnly = (order) => (order?.totalAmount || 0) === 0 && (order?.loyaltyPointsUsed || 0) > 0
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="text-left">
                  ...
                  <div className="flex gap-1 mt-1 flex-wrap justify-end">
                    ...
                    {order.discountCode && (
                      <span className="text-xs px-2 py-0.5 rounded-full border bg-red-500/10 border-red-500/20 text-red-300">
                        كود: {order.discountCode}
                      </span>
                    )}
                    {(order.loyaltyPointsUsed || 0) > 0 && (
                      <span className="text-xs px-2 py-0.5 rounded-full border bg-yellow-500/10 border-yellow-500/20 text-yellow-300">
                        نقاط: {order.loyaltyPointsUsed}
                      </span>
                    )}
                    <span className="text-xs px-2 py-0.5 rounded-full border bg-purple-500/10 border-purple-500/20 text-purple-300">
              ...
              <div className="glass p-4 rounded-2xl space-y-2">
                ...
                <div className="border-t border-white/10 pt-2 mt-2 space-y-1">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-purple-300">{getPaymentMethodLabel(selectedOrder)}</span>
                  </div>
                ...
                {isPaidByLoyaltyOnly(selectedOrder) && (
                  <p className="text-purple-300 text-sm mt-1">💳 تم دفع الطلب بالكامل بواسطة نقاط الولاء ({selectedOrder.loyaltyPointsUsed} نقطة)</p>
                )}
              ...
              <div className="space-y-2">
                {selectedOrder.items?.map((item, i) => (
                  <div key={i} className="glass rounded-xl overflow-hidden">
                    ...
                    {/* Customizations */}
                    {item.customizations && Object.keys(item.customizations).filter(k => item.customizations[k] && k !== 'logoFile').length > 0 && (
                      <div className="px-3 pb-3 border-t border-white/5 pt-2 space-y-2">
                        ...
                        {item.customizations.logoUrl && (
                          <div className="space-y-1">
                            <span className="text-gray-400 text-xs">الشعار:</span>
                            <div className="flex items-center gap-3">
                              <a href={item.customizations.logoUrl} target="_blank" rel="noopener noreferrer">
                                <img src={item.customizations.logoUrl} alt="الشعار" className="w-16 h-16 rounded-lg object-cover border border-purple-500/30 hover:border-purple-500/60 transition-colors" />
                              </a>
                              <div>
                                <a href={item.customizations.logoUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline text-xs" dir="ltr">
                                  {item.customizations.logoFileName || 'عرض الشعار'}
                                </a>
                              </div>
                            </div>
                          </div>
                        ...
                        {item.customizations.primaryColor && (
                          <div className="flex items-center gap-2 text-xs">
                            <span className="text-gray-400">اللون الأساسي:</span>
                            <div className="flex items-center gap-1">
                              <div className="w-4 h-4 rounded border border-white/20" style={{ backgroundColor: item.customizations.primaryColor }} />
                              <span className="text-white/70" dir="ltr">{item.customizations.primaryColor}</span>
                            </div>
                          </div>
                        )}
                        
                        {item.customizations.secondaryColor && (
                          <div className="flex items-center gap-2 text-xs">
                            <span className="text-gray-400">اللون الثانوي:</span>
                            <div className="flex items-center gap-1">
                              <div className="w-4 h-4 rounded border border-white/20" style={{ backgroundColor: item.customizations.secondaryColor }} />
                              <span className="text-white/70" dir="ltr">{item.customizations.secondaryColor}</span>
                            </div>
                          </div>
                        )}
                        ...
                        {item.customizations.notes && (
                          <div className="text-xs">
                            ...
                            <p className="text-white/70 mt-0.5">{item.customizations.notes}</p>
                          </div>
                        )}
