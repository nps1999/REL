    ...
    <div className="product-card group cursor-pointer" onMouseEnter={() => setHovering(true)} onMouseLeave={() => setHovering(false)} data-testid={`product-card-${product.slug || product.id}`}>
      <Link href={`/products/${product.slug || product._id}`}>
        <div className="relative aspect-square overflow-hidden rounded-t-xl bg-[#111827]">
          ...
          {product.featured && <div className="absolute top-2 sm:top-3 left-2 sm:left-3 bg-gradient-to-r from-emerald-500 to-cyan-400 text-white text-[10px] sm:text-xs font-bold px-2 py-0.5 sm:py-1 rounded-full flex items-center gap-0.5"><Zap size={8} /> مميز</div>}
        </div>
      </Link>
...
export default function ProductsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-[#0B0F14]"><div className="spinner mx-auto" /></div>}>
      <ProductsContent />
    </Suspense>
  )
}
